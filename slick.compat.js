...
var DataView = (function () {
    function DataView(options) {
        ...
        this.rowsById = null; // rows by id; lazy-calculated
    ...
    // (fn: (void) => Any) => void
    DataView.prototype.withTransaction = function (fn) {
    ...
    DataView.prototype.setPagingOptions = function (args) {
    ...
    DataView.prototype.insertItem = function (insertBefore, item) {
    ...
    DataView.prototype.deleteItem = function (id) {
    ...
    DataView.prototype.getFlattenedGroups = function (groups, options) {
        if (!options)
            options = {};
        if (options.excludeHiddenGroups == null)
            options.excludeHiddenGroups = false;
    ...
    DataView.prototype.getItem = function (i) {
    ...
    DataView.prototype.getItemMetadata = function (i) {
        var item = this.rows[i];
        if (item === undefined) {
            return null;
        }
     ...
     *     the 'high' group.
     */
    DataView.prototype.collapseGroup = function (varArgs) {
     ...
     *     the 'high' group.
     */
    DataView.prototype.expandGroup = function (varArgs) {
        var args = Array.prototype.slice.call(arguments);
        var arg0 = args[0];
        if (args.length == 1 && arg0.indexOf(this.groupingDelimiter) != -1) {
...
function SlickGrid(container, data, columns, options) {
    ...
    function finishInitialization(newContainer) {
        ...
        /* SlickGrid Dom structure:
          ...
          */
        // ----------------------- Create the elements
        topViewport.el = $("<div class='viewport T L' tabIndex='0' hideFocus />" +
            "<div class='viewport T R' tabIndex='0' hideFocus />");
        topCanvas.el = $("<div class='canvas T L' />" +
            "<div class='canvas T R' />");
        header.el = $("<div class='header' />" +
            "<div class='header' />");
        subHeaders.el = $("<div class='subHeaders' />" +
            "<div class='subHeaders' />");
        if (!options.showSubHeaders) {
            subHeaders.el.hide();
        }
        contentViewportWrap.el = $("<div class='viewport-wrap C L' tabIndex='0' hideFocus />" +
            "<div class='viewport-wrap C R' tabIndex='0' hideFocus />");
        contentViewport.el = $("<div class='viewport C L antiscroll-inner' tabIndex='0' hideFocus />" +
            "<div class='viewport C R antiscroll-inner' tabIndex='0' hideFocus />");
        contentCanvas.el = $("<div class='canvas C L' tabIndex='0' hideFocus />" +
    ...
    function updateCanvasWidth(forceColumnWidthsUpdate) {
        var oldCanvasWidth = contentCanvas.width, oldCanvasWidthL = contentCanvas[0].width, oldCanvasWidthR = contentCanvas[1].width, widthChanged;
        calculateCanvasWidth();
        var canvasWidth = contentCanvas.width, canvasWidthL = contentCanvas[0].width, canvasWidthR = contentCanvas[1].width;
        widthChanged = canvasWidth !== oldCanvasWidth ||
            canvasWidthL !== oldCanvasWidthL ||
            canvasWidthR !== oldCanvasWidthR;
        if (widthChanged || isPinned) {
            //        setHeadersWidth();
    ...
    function setupColumnResize() {
        ...
        columnElements.each(function (i, e) {
            ...
            $("<div class='resizer' />")
                ...
                if (idx > -1) {
                    columns[idx].manuallySized = true;
                }
    ...
    function updateAntiscroll() {
        ...
        var cl = contentViewportWrap.el.filter('.C.L'), cr = contentViewportWrap.el.filter('.C.R');
        if (isPinned) {
            enableAntiscroll(cr);
    ...
    function measureCssSizes() {
        if (!options.rowHeight) {
            var el, markup = "<div class='cell' style='visibility:hidden'>-</div>";
    ...
    function autosizeColumns() {
        var i, c, widths = [], shrinkLeeway = 0, total = 0, prevTotal, availWidth = viewportHasVScroll ? contentViewport.width - scrollbarDimensions.width : contentViewport.width;
    ...
    function invalidateSafe() {
        if (getEditorLock().isActive()) {
            ...
            if (state.invalidateSafeCellChangeCallback) {
                return;
            }
    ...
    function renderRows(range) {
        var markupArrayL = [], markupArrayR = [], rows = [], needToReselectCell = false, dataLength = getDataLength();
    ...
    function reallyHandleScroll(isMouseWheel) {
        ...
        if (hScrollDist || vScrollDist) {
            ...
            if (Math.abs(lastRenderedScrollTop - scrollTop) > 20 ||
                ...
                if (options.forceSyncScrolling || (Math.abs(lastRenderedScrollTop - scrollTop) < contentViewport.height &&
    ...
    function getRowFromNode(node) {
        for (var idx in rowsCache) {
            if (rowsCache[idx].rowNode[0] === node ||
    ...
    function scrollCellIntoView(row, cell, doPaging) {
        ...
        var right = columnPosRight[cell + (colspan > 1 ? colspan - 1 : 0)], scrollRight = scrollLeft + contentViewport.width;
    ...
    function crossesPinnedArea(indices) {
        ...
        var max = Math.max.apply(null, indices), min = Math.min.apply(null, indices);
    ...
    $.extend(this, {
        ...
        "setHeaderRowVisibility": setSubHeadersVisibility,
